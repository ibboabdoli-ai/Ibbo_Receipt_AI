...
async function getPrivateReceipts(): Promise<PrivateReceipt[]> {
  await ensureReceiptsTable();

  const result = await db.execute(
    "SELECT id, date, merchant, amount, currency, category, status FROM receipts WHERE expense_type = 'private' ORDER BY created_at DESC LIMIT 100",
  );
...
export default async function PrivatePage() {
  let receipts: PrivateReceipt[] = [];
  let errorMessage = "";

  try {
    receipts = await getPrivateReceipts();
  } catch (error) {
    console.error("Private page failed", error);
    errorMessage = "Could not load private receipts from Turso.";
  }
  ...
  return (
    <div className="space-y-7">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-soft sm:p-8">
        <p className="text-sm font-bold uppercase tracking-[0.18em] text-slate-400">Private</p>
        <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">Private expenses</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
          Review receipts marked as private costs before export, reports, and monthly cleanup.
        </p>
      </section>

      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
          <p className="text-sm font-semibold text-slate-500">Private total</p>
          <p className="mt-2 text-3xl font-black tracking-tight text-slate-950">{formatCurrency(totalAmount)}</p>
          <p className="mt-4 text-sm leading-6 text-slate-600">Total amount marked as private.</p>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
          ...
          <p className="mt-4 text-sm leading-6 text-slate-600">Private receipts saved in Turso.</p>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
          ...
          <p className="mt-4 text-sm leading-6 text-slate-600">Private receipts that still need review.</p>
        </div>
      </section>
